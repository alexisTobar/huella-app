...
function Marketplace() {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] max-w-7xl mx-auto px-4 pb-32 overflow-x-hidden font-sans">
      
      {/* 1. HERO MARKETPLACE - Full Responsive */}
      <div className="relative bg-emerald-900 rounded-[2.5rem] md:rounded-[3.5rem] p-6 md:p-16 mb-8 md:mb-12 overflow-hidden shadow-2xl border border-white/5 mt-4 md:mt-6">
        <div className="absolute top-0 right-0 w-1/3 h-full bg-emerald-500 skew-x-12 translate-x-20 opacity-20"></div>
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6 md:gap-10 text-center md:text-left">
          <div className="max-w-xl">
            ...
            <h1 className="text-3xl md:text-6xl font-black mb-4 md:mb-6 leading-tight text-white tracking-tighter">
              Todo para tu <br />
              <span className="text-emerald-400">mascota feliz.</span>
            </h1>
            <p className="text-emerald-100/60 text-sm md:text-lg font-medium">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8 md:mb-12">
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          <h2 className="text-2xl md:text-4xl font-black text-slate-800 tracking-tighter uppercase flex items-center gap-3">
          ...
          <div className="relative w-full md:w-80 group">
            <input 
              ...
              className="w-full bg-white border border-slate-200 rounded-2xl py-3 px-10 outline-none font-bold text-slate-600 shadow-sm focus:border-emerald-500 transition-all text-sm"
            />
      ...
      {productosFiltrados.length === 0 ? (
        <div className="text-center py-16 md:py-24 bg-white rounded-[3rem] border-2 border-dashed border-emerald-100 mx-2 animate-fade-in">
          <span className="text-5xl md:text-7xl mb-4 block opacity-50">{busqueda ? '🔍' : '🛒'}</span>
          <p className="text-slate-400 font-black uppercase tracking-widest text-sm md:text-xl">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-8">
          {productosFiltrados.map((prod) => (
            <div key={prod._id} className="bg-white rounded-[1.5rem] md:rounded-[2.5rem] p-2 md:p-4 border border-slate-100 shadow-lg hover:shadow-2xl transition-all duration-500 flex flex-col h-full group">
              ...
              >
                ...
                <div className="absolute top-2 right-2 bg-white/90 backdrop-blur px-2 py-0.5 rounded-full text-[7px] md:text-[10px] font-black text-emerald-700 uppercase tracking-tighter">
                  {prod.categoria}
                </div>
              </div>

              <div className="px-1 md:px-2 flex flex-col flex-grow">
                <h3 className="text-xs md:text-xl font-black text-slate-800 leading-tight mb-2 md:mb-4 group-hover:text-emerald-600 transition-colors line-clamp-2 uppercase tracking-tighter h-8 md:h-14">
                  {prod.titulo}
                </h3>
                
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 md:mb-6 bg-emerald-50 p-2 md:p-3 rounded-xl md:rounded-2xl border border-emerald-100 mt-auto">
                  <span className="text-[7px] md:text-[10px] font-black text-emerald-600 uppercase tracking-widest">Precio</span>
                  <span className="text-sm md:text-2xl font-black text-emerald-700">${Number(prod.precio).toLocaleString('es-CL')}</span>
                </div>

                <a 
                  href={`https://wa.me/${prod.whatsappVendedor}`} 
                  target="_blank" rel="noreferrer"
                  className="flex items-center justify-center gap-1 md:gap-2 w-full bg-slate-900 text-white py-2.5 md:py-4 rounded-xl md:rounded-2xl font-black text-[9px] md:text-xs uppercase tracking-widest hover:bg-emerald-600 transition-all active:scale-95 shadow-md"
                >
      ...
      {modalData.isOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-2 md:p-4 bg-slate-900/95 backdrop-blur-md animate-fade-in" onClick={() => setModalData({ ...modalData, isOpen: false })}>
          <button className="absolute top-4 right-4 md:top-6 md:right-6 w-10 h-10 md:w-12 md:h-12 bg-white rounded-full flex items-center justify-center text-slate-900 text-xl md:text-2xl font-bold shadow-2xl z-[210]">✕</button>
          ...
          <div className="relative max-w-4xl w-full flex flex-col items-center gap-4 md:gap-6" onClick={e => e.stopPropagation()}>
            <div className="relative w-full flex justify-center items-center">
              ...
              <img src={modalData.fotos[modalData.index]} className="max-w-full max-h-[70vh] md:max-h-[85vh] rounded-[1.5rem] md:rounded-[2.5rem] shadow-2xl border-2 md:border-4 border-white/10 object-contain" alt="Producto" />
